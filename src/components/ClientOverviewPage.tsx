...
const ClientOverviewPage = ({ onAddClientClick }: ClientOverviewPageProps) => {
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl text-blue-950 font-bold mb-6">Client Overview</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {clients.map((client) => (
          <div key={client.id} className="bg-white rounded-lg shadow p-4">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-lg text-blue-950 font-bold">{client.name}</h2>
              <button className="ml-2 text-blue-950 hover:text-gray-500" onClick={() => handleDeleteClick(client.id, client.projects[0])}>
                <TrashIcon className="w-6 h-6" />
              </button>
            </div>
            <p className="text-md text-blue-950 mb-2 font-semibold">Associated Projects:</p>
            <ul className="list-disc list-inside pl-2 text-sm text-blue-950">
      ...
      <div className="mt-6">
        <button 
          ...
          className="bg-blue-950 font-bold rounded-lg font-bold text-white hover:text-blue-950 hover:bg-white font-medium py-2 px-4 rounded"
        >
          ADD CLIENT
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="mb-6">
              ...
              <p className="text-blue-950 mt-2">This action cannot be undone.</p>
            </div>
            
            <div className="flex justify-end space-x-3">
              <button
                onClick={handleCloseModal}
                className="text-blue-950 px-6 border border-blue-950 py-2 rounded hover:bg-white hover:text-blue-950 font-medium"
              >
                Back
              </button>
              <button
                onClick={handleConfirmDelete}
                className="bg-blue-950 text-white px-6 py-2 rounded hover:bg-white hover:text-blue-950 hover:border-blue-950"
              >
                Delete
              </button>
            </div>
